...
export function PostCard({ post, currentUserId, detailed = false }: PostCardProps) {
  ...
  return (
    <div className={cn(
      "border-b border-gray-200 dark:border-gray-800 p-4 transition-colors",
      !detailed && "hover:bg-gray-50 dark:hover:bg-gray-900/50"
    )}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-6 text-gray-500 dark:text-gray-400">
            <button onClick={handleLike} className="flex items-center gap-2 hover:text-red-500 transition-colors group">
              <Heart className={cn("h-5 w-5 group-hover:scale-110 transition-transform", post.isLiked && "fill-red-500 text-red-500")} />
              <span className="text-sm">{formatNumber(post.likesCount)}</span>
            </button>
            <Link to={`/posts/${post.id}`} className="flex items-center gap-2 hover:text-blue-500 transition-colors group">
              <MessageCircle className="h-5 w-5 group-hover:scale-110 transition-transform" />
              <span className="text-sm">{formatNumber(post.commentsCount)}</span>
            </Link>
            <button onClick={handleBookmark} className="flex items-center gap-2 hover:text-blue-500 transition-colors group">
              <Bookmark className={cn("h-5 w-5 group-hover:scale-110 transition-transform", post.isBookmarked && "fill-blue-500 text-blue-500")} />
